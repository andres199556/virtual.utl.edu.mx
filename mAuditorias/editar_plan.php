...
if($id == "" || $id == null){
    ...
    //la busco
    $auditoria  =$conexion->query("SELECT id_plan,id_auditoria,id_proceso,id_direccion,id_responsable,fecha_hora_plan,elementos,(SELECT GROUP_CONCAT(AP.id_usuario) FROM auditores_planes as AP WHERE AP.id_plan = planes_auditorias.id_plan) as auditores FROM planes_auditorias WHERE id_plan = $id AND activo = 1");
    $existe = $auditoria->rowCount();
    if($existe == 0){
        //header("Location:index.php");
    }
    else{
        $row_datos = $auditoria->fetch(PDO::FETCH_ASSOC);
        $auditores = $row_datos["auditores"];
        $id_auditoria = $row_datos["id_auditoria"];
        $array_auditores = explode(",",$auditores);
    }
}
?>
<!DOCTYPE html>
...
<body class="fix-sidebar fix-header card-no-border">
    ...
    <div id="main-wrapper">
        ...
        <div class="page-wrapper">
            ...
            <div class="container-fluid">
                ...
                <!-- Start Page Content -->
                <!-- ============================================================== -->
                <form action="actualizar_plan.php" method="post" id="frmAlta">
                <input type="hidden" name="id_plan" value="<?php echo $id;?>">
                <input type="hidden" name="id_auditoria" value="<?php echo $id_auditoria;?>">
                    <div class="row">
                        <div class="col-12">
                            <div class="card card-outline-info">
                                <div class="card-header">
                                    <strong><span class="text-white">Editar plan de auditoría</span></strong>
                                </div>
                                <div class="card-body">
                                    <div class="row">
                                        ...
                                        <div class="col-md-3 form-group">
                                            ...
                                            <input type="text" name="elementos" required id="elementos" value="<?php echo $row_datos["elementos"];?>" class="form-control">
                                        </div>
                                    </div>
                                    <div class="row">
                                        <div class="col-md-4 form-group">
                                            <label for="fecha_hora" class="control-label">Fecha y hora:</label>
                                            <input type="text" name="fecha_hora" value="<?php echo $row_datos["fecha_hora_plan"];?>" fechas required id="fecha_hora" class="form-control">
                                        </div>
                                ...
                                <div class="card-footer">
                                    <div class="row">
                                        ...
                                        <div class="col-md-2">
                                            <button type="submit" class="btn btn-success btn-block">Actualizar plan</button>
                                        </div>
                                    </div>
                                </div>
                            </div>
    ...
    $("#direccion").val(<?php echo $row_datos["id_direccion"];?>);
    $("#proceso").val(<?php echo $row_datos["id_proceso"];?>);
    $("#responsable").val(<?php echo $row_datos["id_responsable"];?>);
    <?php
    foreach($array_auditores as $id_auditor){
        ?>
        $("#auditores option[value='<?php echo $id_auditor;?>']").prop("selected", true);
        <?php
    }
    ?>
    $("select").select2(
